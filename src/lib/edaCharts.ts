...
export const SINGLE_NUM = ["Histogram", "KDE (density)", "Box", "Violin", "ECDF", "Strip / jitter", "QQ plot", "Cumulative histogram"];
...
export const COMPARE_CHARTS = [
  ...
  "Box by group", "Violin by group", "Grouped bar", "Stacked bar", "Parallel coordinates", "Missingness bar", "Outlier summary",
];
...
function buildSingle(ds: Dataset, spec: EdaSpec, theme: Theme): ChartFig {
  ...
  if (chart === "QQ plot") {
    const s=[...present].sort((a,b)=>a-b), n=s.length; const m=s.reduce((a,b)=>a+b,0)/(n||1); const sd=Math.sqrt(s.reduce((a,b)=>a+(b-m)**2,0)/(n||1))||1; const normal=(p:number)=>{ let lo=-8,hi=8; for(let i=0;i<30;i++){const z=(lo+hi)/2; const c=0.5*(1+Math.tanh(z/Math.sqrt(2))); if(c<p) lo=z; else hi=z;} return (lo+hi)/2; }; const theo=s.map((_,i)=>normal((i+0.5)/n)); const actual=s.map(v=>(v-m)/sd);
    return {data:[{type:"scatter",x:theo,y:actual,mode:"markers",marker:{color:theme.accent,size:6},name:"quantiles"},{type:"scatter",x:[Math.min(...theo),Math.max(...theo)],y:[Math.min(...theo),Math.max(...theo)],mode:"lines",line:{color:theme.muted,dash:"dot"},name:"reference"}],layout:base(theme,`${col.name} — Q-Q plot`,{xaxis:{title:"theoretical quantiles"},yaxis:{title:"sample quantiles"}})};
  }

  if (chart === "Cumulative histogram") {
    const s=[...present].sort((a,b)=>a-b), y=s.map((_,i)=>i+1); return {data:[{type:"scatter",x:s,y,mode:"lines",fill:"tozeroy",line:{color:theme.accent}}],layout:base(theme,`${col.name} — cumulative histogram`,{yaxis:{title:"cumulative count"}})};
  }
...
function buildCompare(ds: Dataset, spec: EdaSpec, theme: Theme): ChartFig {
  ...
  if (chart === "Missingness bar") {
    const labels=ds.columns.map(c=>c.name), vals=ds.columns.map(c=>c.values.filter(v=>v==null).length); return {data:[{type:"bar",x:labels,y:vals,marker:{color:theme.accent}}],layout:base(theme,"Missing values by column",{xaxis:{automargin:true},yaxis:{title:"missing cells"}})};
  }
  if (chart === "Outlier summary") {
    const labels:string[]=[], vals:number[]=[]; for(const c of ds.columns.filter(c=>c.type==="num")){const a=c.values.filter((v):v is number=>v!=null),ss=[...a].sort((x,y)=>x-y); if(!a.length) continue; const q1=ss[Math.floor(.25*(ss.length-1))],q3=ss[Math.floor(.75*(ss.length-1))],iqr=q3-q1; labels.push(c.name); vals.push(a.filter(v=>v<q1-1.5*iqr||v>q3+1.5*iqr).length);} return {data:[{type:"bar",x:labels,y:vals,marker:{color:theme.accent}}],layout:base(theme,"Outlier count by numeric feature",{xaxis:{automargin:true},yaxis:{title:"outlier rows"}})};
  }
