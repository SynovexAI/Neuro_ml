...
export async function GET() {
  ...
  const out = rows.map((r) => ({ id: r.id, type: r.type, projectId: r.projectId, agentName: nameOf.get(r.projectId) || "(deleted agent)", enabled: r.enabled, hasSecret: !!r.secretEnc, createdAt: r.createdAt }));
...
export async function POST(req: Request) {
  ...
  if (type === "api") {
    ...
    await db.insert(channels).values({ id, userId: user.id, projectId, type, secretEnc: encrypt(key) });
    // Returned once, in plaintext — the caller must copy it now.
    return NextResponse.json({ ok: true, id, apiKey: key });
  }

  // widget — no secret
  await db.insert(channels).values({ id, userId: user.id, projectId, type });
