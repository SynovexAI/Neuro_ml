...
// Connect to a user-supplied MySQL/TiDB or Postgres URL, run a read-only SELECT, return CSV.
  ...
  try {
    ...
    if (isPg) {
      ...
      try {
        ...
        return NextResponse.json({ csv: toCsv(res.rows as Record<string, unknown>[]), rows: res.rows.length });
