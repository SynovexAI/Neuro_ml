  ...
  return (
    <div>
      ...
      {d.kind === "chunk" && <>
        ...
        <div className="insp-field"><div className="k">Chunk size (words) · {d.size}</div><input type="range" min={15} max={1000} step={5} value={d.size ?? 40} onChange={(e) => patchSel({ size: +e.target.value })} /></div>
        <div className="insp-field"><div className="k">Overlap (words) · {d.overlap}</div><input type="range" min={0} max={500} step={2} value={Math.min(d.overlap ?? 8, (d.size ?? 40) - 1)} onChange={(e) => patchSel({ overlap: Math.min(+e.target.value, (d.size ?? 40) - 1) })} /></div>
