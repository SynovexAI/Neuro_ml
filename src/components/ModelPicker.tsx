...
export default function ModelPicker({ models, presetModels = [], value, onChange, placeholder, openKey }: { models: string[]; presetModels?: string[]; value: string; onChange: (v: string) => void; placeholder?: string; openKey?: number }) {
  ...
  const availableModels = models.length > 0 ? models : presetModels;
  const ql = q.trim().toLowerCase();
  const filtered = ql ? availableModels.filter((m) => m.toLowerCase().includes(ql)) : availableModels;
  ...
  return (
    <div style={{ position: "relative", width: "100%" }}>
      <input
        ...
        style={{ width: "100%" }}
      />
      {open && rect && availableModels.length > 0 && typeof document !== "undefined" && createPortal(
