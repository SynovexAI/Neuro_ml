...
import {
  ...
  engineerColumn, ENGINEER_OPS, ENGINEER_NEEDS_B, rankFeaturesByCorrelation, aggregateFeatureScores,
  gridSearch, randomSearch, bayesianSearch, defaultGrid, permutationImportance,
  kMeans, dbscan, agglomerativeClustering, type ClusterResult,
  suggestFeatures, autoDetectPipeline, type FeatureSuggestion, type AutoPipelinePlan,
  type Dataset, type Task, type PrepStep, type TrainConfig, type ClsMetrics, type RegMetrics, type Snapshot,
  type FoldResult, type TreeNode, type BuiltData, type Model, type EngineerOp, type FeatureScore,
  type GridSearchResult, type PermImportance,
} from "@/lib/mlUtils";
import { shapExplain, limeExplain, driftReport, makeVersionRecord, promoteChallenger, type ShapResult, type LimeResult, type VersionRecord } from "@/lib/mlLifecycle";
...
import {
  snapshotSchema, validateDataset, overallValidationStatus, assessQuality, detectLeakage,
  type SchemaSnapshot, type ValidationIssue, type Severity, type QualityScore, type LeakageWarning,
} from "@/lib/dataQuality";

type Step = "problem" | "data" | "validate" | "eda" | "quality" | "leakage" | "prep" | "model" | "validation" | "train" | "deploy";
type ProblemType = "classification" | "regression" | "clustering" | "timeseries";
type ParamSpec = { name: string; type: "num" | "sel"; def: number | string; min?: number; max?: number; step?: number; opts?: string[] };
type ModelCompareRow = { algo: string; score: number; std: number; ms: number; error?: string };
...
const MODELS: Record<Task, Record<string, ParamSpec[]>> = {
  classification: {
    ...
    GaussianNB: [],
    SVMClassifier: [{ name: "C", type: "num", def: 1, min: 0.01, max: 10, step: 0.01 }, { name: "max_iter", type: "num", def: 300, min: 50, max: 1000, step: 50 }, { name: "learning_rate", type: "num", def: 0.05, min: 0.001, max: 0.2, step: 0.001 }],
    DecisionTree: TREE_PARAMS,
    GradientBoosting: [{ name: "n_estimators", type: "num", def: 25, min: 5, max: 100, step: 5 }, { name: "max_depth", type: "num", def: 2, min: 1, max: 5, step: 1 }, { name: "learning_rate", type: "num", def: 0.1, min: 0.01, max: 0.5, step: 0.01 }],
  ...
  regression: {
    ...
    Lasso: [{ name: "alpha", type: "num", def: 0.1, min: 0, max: 10, step: 0.01 }],
    SVR: [{ name: "C", type: "num", def: 1, min: 0.01, max: 10, step: 0.01 }, { name: "epsilon", type: "num", def: 0.1, min: 0.001, max: 1, step: 0.01 }],
...
const MODEL_INFO: Record<string, { label: string; family: string; desc: string }> = {
  ...
  SVMClassifier: { label: "Support Vector Machine", family: "Margin", desc: "Linear maximum-margin classifier trained with hinge loss." },
  GradientBoosting: { label: "Gradient Boosting", family: "Ensemble", desc: "Sequential shallow trees that reduce residual error." },
  Lasso: { label: "Lasso Regression", family: "Linear", desc: "Linear regression with L1-style regularization." },
  SVR: { label: "Support Vector Regression", family: "Margin", desc: "Epsilon-insensitive regression with linear support vectors." },
};
const OPS: Record<string, { methods: string[]; type: "any" | "num" | "cat"; hint: string }> = {
  "Impute missing": { methods: ["Mean", "Median", "Most frequent", "Constant", "Min", "Max", "Forward fill", "Backward fill", "Interpolate", "KNN impute", "Iterative impute"], type: "any", hint: "Fill gaps (nulls) so no value is missing." },
  ...
  "Encode categorical": { methods: ["One-Hot", "Ordinal", "Frequency", "Count", "Binary", "Label"], type: "cat", hint: "Turn text categories into numbers a model can use." },
  ...
  "Transform": { methods: ["Log", "Log1p", "Sqrt", "Square", "Cube root", "Reciprocal", "Absolute", "Yeo-Johnson", "Box-Cox", "Rank"], type: "num", hint: "Reshape a distribution (e.g. reduce skew)." },
  "Bin / discretize": { methods: ["Equal-width (5)", "Equal-freq (5)", "Equal-width (10)"], type: "num", hint: "Bucket a numeric column into ordered bins." },
  "Polynomial features": { methods: ["degree 2", "degree 3"], type: "num", hint: "Add polynomial powers of selected numeric features." },
  "Interaction features": { methods: ["pairwise product"], type: "num", hint: "Create interaction terms between selected numeric features." },
  "Missing indicators": { methods: ["0/1 indicator"], type: "any", hint: "Add a flag showing which rows were originally missing." },
  "Date/time features": { methods: ["Year", "Month", "Day", "Day of week", "Quarter", "Weekend"], type: "cat", hint: "Extract useful calendar features from date-like columns." },
  "Text features": { methods: ["Word count", "Character count", "Unique token count"], type: "cat", hint: "Extract simple numeric signals from text columns." },
...
const METHOD_DESC: Record<string, string> = {
  ...
  Binary: "encodes the category index in binary across bit-columns", Label: "maps categories to stable integer labels",
  ...
  "Cube root": "applies the cube root", Reciprocal: "applies 1/x", Absolute: "takes the absolute value", "Yeo-Johnson": "power transform that supports zero and negative values", "Box-Cox": "power transform for positive values", Rank: "replaces values by normalized rank",
  "Equal-width (5)": "buckets into 5 equal-width bins", "Equal-freq (5)": "buckets into 5 equal-frequency (quantile) bins",
  "Equal-width (10)": "buckets into 10 equal-width bins", "degree 2": "adds x²", "degree 3": "adds x² and x³", "pairwise product": "multiplies selected numeric features", "0/1 indicator": "adds a missingness flag", Year: "extracts calendar year", Month: "extracts calendar month", Day: "extracts day of month", "Day of week": "extracts weekday index", Quarter: "extracts quarter", Weekend: "extracts weekend flag", "Word count": "counts whitespace-separated words", "Character count": "counts characters", "Unique token count": "counts unique whitespace tokens", drop: "removes the column",
};
...
export default function MlLab() {
  ...
  const [learnTopic, setLearnTopic] = useState<Step | "hyperparameters" | null>(null);

  const LEARN_TOPICS: Record<Step | "hyperparameters", { title: string; what: string; why: string; how: string; takeaway: string }> = {
    problem: { title: "Problem Definition", what: "Define what you want the ML system to learn or discover and choose the problem type.", why: "The problem type controls which models, validation methods, and evaluation metrics are appropriate.", how: "Choose classification for categories, regression for numeric values, or clustering when you want to discover groups without a target.", takeaway: "A clear problem definition is the starting point for a correct ML workflow." },
    data: { title: "Data", what: "Bring a sample, CSV, or database dataset into the ML workspace.", why: "Every later ML step depends on understanding the data you are working with.", how: "Load the dataset, inspect its rows and columns, identify data types, and select the target/features when needed.", takeaway: "Good ML starts with understanding the data before changing or modeling it." },
    validate: { title: "Data Validation", what: "Check whether the uploaded dataset is structurally and logically suitable for ML.", why: "Invalid values, missing information, or schema problems can produce unreliable models or errors.", how: "The validator checks dataset-level and column-level rules and reports errors, warnings, and passed checks.", takeaway: "Validate data before trusting any model result." },
    eda: { title: "Exploratory Data Analysis", what: "Explore distributions, relationships, categories, correlations, and unusual observations using statistics and charts.", why: "EDA helps students understand what the data is saying before preprocessing or modeling.", how: "Use histograms, box plots, violin plots, bar charts, scatter plots, correlation views, and other visualizations.", takeaway: "EDA turns a table of numbers into understandable patterns." },
    quality: { title: "Data Quality", what: "Measure the overall quality of the dataset using completeness, validity, uniqueness, and consistency.", why: "A model can only learn reliably when the input data is reasonably trustworthy.", how: "Review the quality score, dimension scores, findings, and recommendations before continuing.", takeaway: "Quality assessment tells you where the dataset needs attention." },
    leakage: { title: "Data Leakage Detection", what: "Look for features that could give the model information it would not have at real prediction time.", why: "Leakage can make validation scores look excellent while the model fails in real use.", how: "Review suspicious target-like, future-looking, or outcome-derived columns and exclude risky features when appropriate.", takeaway: "Leakage detection prevents the model from accidentally learning the answer." },
    prep: { title: "Preprocessing", what: "Transform raw data into a clean numeric representation that ML algorithms can use.", why: "Missing values, categories, scales, outliers, and raw formats can prevent algorithms from learning effectively.", how: "Build a sequence of steps such as imputation, encoding, scaling, outlier handling, transformations, binning, and feature engineering.", takeaway: "Preprocessing is the bridge between raw data and model-ready data." },
    model: { title: "Models & Clustering", what: "Choose a supervised learning algorithm or discover natural groups with clustering.", why: "Different algorithms learn different kinds of patterns and have different strengths and assumptions.", how: "For prediction, compare classification or regression models. For unsupervised learning, try K-Means, DBSCAN, or Agglomerative Clustering.", takeaway: "Model choice should match the problem and the structure of the data." },
    validation: { title: "Model Validation", what: "Estimate how well a model is likely to perform on unseen data.", why: "A model can perform well on training data but fail to generalize.", how: "Use K-Fold, Stratified K-Fold, Repeated K-Fold, Time-Series, Group K-Fold, or Hold-Out validation as appropriate.", takeaway: "Validation gives evidence that a model is learning useful patterns rather than memorizing." },
    train: { title: "Model Training", what: "Fit the selected algorithm to the prepared training data.", why: "Training is where the model learns relationships between input features and the target.", how: "Run training after preprocessing and validation choices are ready, then inspect metrics, learning behavior, and feature importance.", takeaway: "Training creates the fitted model that can make predictions." },
    deploy: { title: "Test & Export", what: "Use the trained model on new inputs and save useful outputs.", why: "Testing demonstrates how the trained model behaves on unseen records, while export makes results reusable.", how: "Enter or provide test values, predict, inspect the result and explanation, then export the relevant data/model artifacts.", takeaway: "A model becomes useful when it can make and explain predictions on new data." },
    hyperparameters: { title: "Hyperparameter Tuning", what: "Search for model settings that produce strong validation performance.", why: "Hyperparameters such as tree depth, number of trees, C, alpha, or neighbors affect how a model learns.", how: "Grid Search tests a defined set systematically; Random Search samples combinations; Bayesian Optimization uses earlier results to focus the search. Each candidate is scored with cross-validation.", takeaway: "Tuning does not change the algorithm itself—it searches for better settings for the current dataset." }
  };

  function LearnButton({ topic, compact = false }: { topic: Step | "hyperparameters"; compact?: boolean }) {
    return <button className={`btn ${compact ? "ghost sm" : "ghost"}`} onClick={() => setLearnTopic(topic)} title={`Learn about ${LEARN_TOPICS[topic].title}`}>📚 Learn</button>;
  }
  ...
  // ── Data Validation (§3) ──
  const [refSchema, setRefSchema] = useState<SchemaSnapshot | null>(null);
  // ── Leakage Detection (§7) ──
  const [leakageExcluded, setLeakageExcluded] = useState<Set<string>>(new Set());
  ...
  const [valMethod, setValMethod] = useState<"kfold" | "holdout" | "stratified" | "repeated" | "timeseries" | "group">("kfold");
  const [cvRepeats, setCvRepeats] = useState(3);
  const [cvGroupCol, setCvGroupCol] = useState("");
  ...
  const [result, setResult] = useState<{ metrics: ClsMetrics | RegMetrics; importance: { name: string; w: number }[] | null; permImp: PermImportance[]; cv: number[]; predActual?: [number, number][]; probaTest?: { yte: number[]; proba: number[][] }; loss?: number[]; ms: number } | null>(null);
  ...
  const [shapOut, setShapOut] = useState<ShapResult | null>(null);
  const [limeOut, setLimeOut] = useState<LimeResult | null>(null);
  const [driftMethod, setDriftMethod] = useState<"PSI" | "KS" | "JS" | "Wasserstein">("PSI");
  const [driftOut, setDriftOut] = useState<{ feature: string; score: number; drifted: boolean; threshold: number }[]>([]);
  ...
  // Large datasets can contain millions of cells. Keep expensive browser-side analytics
  // responsive by using a deterministic row sample for dashboards/validation only.
  // The original dataset is never modified and the full dataset remains available for training.
  const ANALYTICS_ROWS = 20000;
  const EDA_ROWS = 8000;
  const analysisDs = useMemo<Dataset | null>(() => {
    if (!ds || ds.nrows <= ANALYTICS_ROWS) return ds;
    const step = ds.nrows / ANALYTICS_ROWS;
    const idx = Array.from({ length: ANALYTICS_ROWS }, (_, i) => Math.min(ds.nrows - 1, Math.floor(i * step)));
    return { nrows: idx.length, columns: ds.columns.map((c) => ({ ...c, values: idx.map((i) => c.values[i]) })) };
  }, [ds]);
  const edaDs = useMemo<Dataset | null>(() => {
    if (!ds || ds.nrows <= EDA_ROWS) return ds;
    const step = ds.nrows / EDA_ROWS;
    const idx = Array.from({ length: EDA_ROWS }, (_, i) => Math.min(ds.nrows - 1, Math.floor(i * step)));
    return { nrows: idx.length, columns: ds.columns.map((c) => ({ ...c, values: idx.map((i) => c.values[i]) })) };
  }, [ds]);
  const desc = useMemo(() => (analysisDs ? describe(analysisDs) : []), [analysisDs]);
  const insights = useMemo(() => (analysisDs ? datasetInsights(analysisDs, target) : []), [analysisDs, target]);
  // ── Data Validation / Quality / Leakage (computed from a bounded analytics sample for large files) ──
  const validationIssues = useMemo<ValidationIssue[]>(() => (analysisDs ? validateDataset(analysisDs, target, refSchema) : []), [analysisDs, target, refSchema]);
  const validationStatus = useMemo<Severity>(() => overallValidationStatus(validationIssues), [validationIssues]);
  const qualityScore = useMemo<QualityScore | null>(() => (analysisDs ? assessQuality(analysisDs, target) : null), [analysisDs, target]);
  const leakageWarnings = useMemo<LeakageWarning[]>(() => (analysisDs && target ? detectLeakage(analysisDs, target, task) : []), [analysisDs, target, task]);
  function excludeLeakyFeature(col: string) { setFeatures((f) => f.filter((x) => x !== col)); setLeakageExcluded((s) => new Set(s).add(col)); }
  function reincludeFeature(col: string) { setFeatures((f) => (f.includes(col) ? f : [...f, col])); setLeakageExcluded((s) => { const n = new Set(s); n.delete(col); return n; }); }
  const edaFig = useMemo(() => {
    if (!edaDs) return null;
    ...
    return buildFigure(edaDs, spec, plotlyTheme());
  }, [edaDs, edaMode, uniCol, uniChart, bins, groupBy, cmpChart, xCol, yCol, colorCol, multiCols, trend]);
  ...
  const built = useMemo<BuiltData | null>(() => {
    if (!ds || !features.length) return null;
    if (problemType === "clustering") { try { return buildMatrix(ds, features, "", "regression", steps); } catch { return null; } }
    if (!target) return null;
    try { return buildMatrix(ds, features, target, task, steps); } catch { return null; }
  }, [ds, target, features, task, steps, problemType]);
  ...
  function runExplainability() {
    if (!modelRef.current || !built || !built.X.length) return;
    const row = built.X[0];
    setShapOut(shapExplain(modelRef.current, row, built.X.slice(0, Math.min(120, built.X.length)), built.featureNames, task, 128, 1));
    setLimeOut(limeExplain(modelRef.current, row, built.X.slice(0, Math.min(120, built.X.length)), built.featureNames, task, 256, undefined, 1));
  }
  function runDriftCheck() {
    if (!built || built.X.length < 10) return;
    const cut = Math.max(2, Math.floor(built.X.length * 0.7));
    const rows = built.X.slice(cut);
    const out = built.featureNames.map((feature, j) => { const r = driftReport(built.X.slice(0, cut).map(x => x[j]), rows.map(x => x[j]), driftMethod); return { feature, score: r.score, drifted: r.drifted, threshold: r.threshold }; }).sort((a,b)=>b.score-a.score);
    setDriftOut(out);
  }
  function saveLifecycleVersion() {
    if (!result) return;
    const metrics: Record<string, number> = {};
    if (result.metrics.task === "classification") {
      metrics.Accuracy = result.metrics.accuracy;
      metrics.Precision = result.metrics.precision;
      metrics.Recall = result.metrics.recall;
      metrics.F1 = result.metrics.f1;
    } else {
      metrics.MAE = result.metrics.mae;
      metrics.RMSE = result.metrics.rmse;
      metrics["R²"] = result.metrics.r2;
    }
    const rec = makeVersionRecord({ name: projName || dsName || "ML model", version: `v${versions.length + 1}`, datasetVersion: `${dsName || "dataset"}:${ds?.nrows ?? 0}x${ds?.columns.length ?? 0}`, features: [...(built?.featureNames ?? features)], preprocessing: steps, algorithm: algo, hyperparameters: { ...params }, metrics, status: "Validation" });
    const next = [...versions, rec]; setVersions(next); setLifecycleMsg(`Saved ${rec.version} to the local model registry.`);
    try { localStorage.setItem("neuro-ml-model-registry", JSON.stringify(next)); } catch {}
  }
  function loadLifecycleVersions() { try { const raw = localStorage.getItem("neuro-ml-model-registry"); if (raw) setVersions(JSON.parse(raw)); } catch {} }
  function compareLatestVersions() {
    if (versions.length < 2) { setLifecycleMsg("Save at least two model versions to compare champion vs challenger."); return; }
    const champion = versions[versions.length - 2], challenger = versions[versions.length - 1];
    const requested = primaryMetric.trim().toLowerCase();
    const metric = task === "classification"
      ? (requested.includes("f1") ? "F1" : requested.includes("precision") ? "Precision" : requested.includes("recall") ? "Recall" : "Accuracy")
      : (requested.includes("mae") ? "MAE" : requested.includes("mse") ? "MSE" : requested.includes("rmse") ? "RMSE" : "R²");
    const higher = !["MAE", "MSE", "RMSE"].includes(metric);
    const c = promoteChallenger(champion, challenger, metric, higher);
    setLifecycleMsg(`Champion/challenger (${metric}): ${c.reason}`);
  }
  useEffect(() => { loadLifecycleVersions(); }, []);
  ...
  function loadCSV(text: string, name: string, tgt?: string, tk?: Task) {
    ...
    setDs(parsed); setDsName(name); setMsg(""); setResult(null); setSnaps([]); setFsScores(null); setTuneResults([]);
  ...
  function loadSample() {
    const sample = sampleDatasets().find((d) => d.key === sampleKey);
    if (!sample) {
      setMsg(`Sample dataset "${sampleKey}" was not found.`);
      return;
    }
    loadCSV(sample.csv, sample.label || sample.key, undefined, undefined);
    setDataTab("sample");
    setMsg(`Loaded sample dataset: ${sample.label || sample.key}`);
  }
  ...
  function pickTarget(name: string) {
    ...
    setFsScores(null); setTuneResults([]);
  }
  ...
  async function gzipBlob(file: File): Promise<Blob> {
    if (typeof CompressionStream === "undefined") throw new Error("Your browser does not support GZIP compression. Please use a recent Chrome, Edge, or Firefox browser.");
    const cs = new CompressionStream("gzip");
    const stream = file.stream().pipeThrough(cs);
    return await new Response(stream).blob();
  }

  async function gunzipText(file: File): Promise<string> {
    if (typeof DecompressionStream === "undefined") throw new Error("Your browser does not support GZIP decompression. Please use a recent Chrome, Edge, or Firefox browser.");
    const ds = new DecompressionStream("gzip");
    const stream = file.stream().pipeThrough(ds);
    return await new Response(stream).text();
  }

  async function compressDatasetFile(file: File) {
    const MAX_UPLOAD_BYTES = 50 * 1024 * 1024;
    setMsg("");
    try {
      if (!/\.(csv|tsv)$/i.test(file.name)) throw new Error("The compressor currently supports CSV and TSV datasets.");
      const compressed = await gzipBlob(file);
      const before = file.size;
      const after = compressed.size;
      const ratio = before ? (after / before) * 100 : 0;
      const saved = before ? ((1 - after / before) * 100) : 0;
      const outName = `${file.name}.gz`;

      if (after > MAX_UPLOAD_BYTES) {
        setMsg(`❌ Compression completed, but the result is ${formatBytes(after)}, which is above the 50 MB upload limit. The original dataset was not changed and no upload file was downloaded. Try a dataset that compresses to 50 MB or less.`);
        return;
      }

      // Keep the compressed bytes in memory and load them directly into ML Lab.
      // This avoids the common mistake of selecting the original oversized CSV
      // again in the upload control. The original file is never modified.
      const compressedFile = new File([compressed], outName, { type: "application/gzip", lastModified: Date.now() });
      const text = await gunzipText(compressedFile);
      if (!text.trim()) throw new Error("Compression produced an empty dataset.");
      loadCSV(text, file.name, undefined, undefined);
      ...
      setMsg(`✅ Compressed and loaded: ${formatBytes(before)} → ${formatBytes(after)} (${saved >= 0 ? `${saved.toFixed(1)}% smaller` : "larger after compression"}). The compressed ${outName} was also downloaded. ML Lab is using the decompressed original contents.`);
    } catch (e) { setMsg((e as Error).message || "Could not compress the dataset."); }
  }
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setMsg("");
    try {
      // The platform upload limit applies to the file selected here. The compressor
      // is separate, so an oversized CSV can still be selected in the compressor.
      if (f.size > MAX_UPLOAD_BYTES) {
        throw new Error(`Upload rejected: ${formatBytes(f.size)} exceeds the 50 MB limit. Use the Dataset Compressor first and upload the resulting .gz file only when it is ≤ 50 MB.`);
      }
      if (/\.(gz|gzip)$/i.test(f.name)) {
        const text = await gunzipText(f);
        if (!text.trim()) throw new Error("The compressed dataset is empty.");
        loadCSV(text, f.name.replace(/\.(gz|gzip)$/i, ""));
      } else if (/\.(csv|tsv)$/i.test(f.name)) {
        const text = await f.text();
        if (!text.trim()) throw new Error("The dataset file is empty.");
        loadCSV(text, f.name);
      } else {
        throw new Error("Unsupported dataset format. Please upload CSV, TSV, or a GZIP-compressed CSV/TSV file.");
      }
    } catch (err) { setMsg((err as Error).message || "Could not read the dataset."); }
  ...
  // ── feature engineering: derive a new numeric column from existing ones ──
  const [engOp, setEngOp] = useState<EngineerOp>(ENGINEER_OPS[0]);
  const [engColA, setEngColA] = useState("");
  const [engColB, setEngColB] = useState("");
  const numEngCols = ds ? ds.columns.filter((c) => c.type === "num" && c.name !== target).map((c) => c.name) : [];
  function addEngineeredFeature() {
    if (!ds || !engColA) return;
    try {
      const needsB = ENGINEER_NEEDS_B[engOp];
      const col = engineerColumn(ds, engColA, needsB ? (engColB || null) : null, engOp);
      if (ds.columns.some((c) => c.name === col.name)) { setMsg(`Column "${col.name}" already exists.`); return; }
      setDs({ nrows: ds.nrows, columns: [...ds.columns, col] });
      setFeatures((f) => [...f, col.name]);
      setSteps((s) => [...s, { op: "Impute missing", cols: [col.name], method: "Mean" }, { op: "Scale / normalize", cols: [col.name], method: "StandardScaler" }]);
      setMsg(`Added engineered feature "${col.name}".`);
      setEngColA(""); setEngColB("");
    } catch (e) { setMsg((e as Error).message); }
  }

  // ── feature selection: rank encoded features by |corr| with target, keep top-N ──
  const [fsScores, setFsScores] = useState<FeatureScore[] | null>(null);
  const [fsTopN, setFsTopN] = useState(5);
  function runFeatureSelection() {
    if (!built) { setMsg("Pick a target & at least one feature first."); return; }
    setFsScores(aggregateFeatureScores(rankFeaturesByCorrelation(built.X, built.y, built.featureNames)));
  }
  function applyTopFeatures() {
    if (!fsScores) return;
    setFeatures(fsScores.slice(0, Math.max(1, Math.min(fsTopN, fsScores.length))).map((s) => s.name));
  }

  // ── AI-suggested feature engineering: rule-based scan for promising derived columns ──
  const [featureSuggestions, setFeatureSuggestions] = useState<FeatureSuggestion[] | null>(null);
  const [suggestBusy, setSuggestBusy] = useState(false);
  function runFeatureSuggestions() {
    if (!ds || !target) { setMsg("Pick a target column first (Data step)."); return; }
    setSuggestBusy(true);
    setTimeout(() => {
      try { setFeatureSuggestions(suggestFeatures(ds, target)); }
      catch (e) { setMsg((e as Error).message); }
      setSuggestBusy(false);
    }, 20);
  }
  function applyFeatureSuggestion(s: FeatureSuggestion) {
    if (!ds) return;
    try {
      const col = engineerColumn(ds, s.colA, s.colB, s.op);
      if (ds.columns.some((c) => c.name === col.name)) { setMsg(`Column "${col.name}" already exists.`); return; }
      setDs({ nrows: ds.nrows, columns: [...ds.columns, col] });
      setFeatures((f) => [...f, col.name]);
      setSteps((st) => [...st, { op: "Impute missing", cols: [col.name], method: "Mean" }, { op: "Scale / normalize", cols: [col.name], method: "StandardScaler" }]);
      setFeatureSuggestions((list) => list ? list.filter((x) => x.id !== s.id) : list);
      setMsg(`Added suggested feature "${col.name}".`);
    } catch (e) { setMsg((e as Error).message); }
  }

  // ── auto pipeline detection: one-click target/task/features/preprocessing, previewed before applying ──
  const [autoPlan, setAutoPlan] = useState<AutoPipelinePlan | null>(null);
  function runAutoDetect() {
    if (!ds) { setMsg("Load a dataset first."); return; }
    setAutoPlan(autoDetectPipeline(ds));
  }
  function applyAutoPlan() {
    if (!ds || !autoPlan) return;
    setTarget(autoPlan.target); setTask(autoPlan.task); setFeatures(autoPlan.features); setSteps(autoPlan.steps);
    const a = Object.keys(MODELS[autoPlan.task])[0]; setAlgo(a); setParamsFor(autoPlan.task, a);
    setResult(null); setSnaps([]); setFsScores(null); setTuneResults([]); setFeatureSuggestions(null);
    setMsg(`Applied auto-detected pipeline: target "${autoPlan.target}" (${autoPlan.task}), ${autoPlan.features.length} features, ${autoPlan.steps.length} preprocessing steps.`);
    setAutoPlan(null);
    setStep("prep");
  }
  ...
  function buildFullReportMarkdown(): string {
    const lines: string[] = [];
    const now = new Date().toLocaleString();
    lines.push(`# ${projName || "ML Project"} — Full Report`, "", `Generated: ${now}`, "");
    if (problemStatement || bizObjective || mlObjective) {
      lines.push("## Problem Definition", "");
      if (problemStatement) lines.push(`**Problem statement:** ${problemStatement}`, "");
      if (bizObjective) lines.push(`**Business objective:** ${bizObjective}`, "");
      if (mlObjective) lines.push(`**ML objective:** ${mlObjective}`, "");
      lines.push(`**Problem type:** ${problemType}`, `**Primary metric:** ${primaryMetric || "—"}`, "");
    }
    if (ds) {
      const missing = ds.columns.reduce((n, c) => n + c.values.filter((v) => v == null || v === "").length, 0);
      const cells = Math.max(1, ds.nrows * ds.columns.length);
      lines.push("## Dataset", "", `- Rows: ${ds.nrows}`, `- Columns: ${ds.columns.length}`, `- Missing cells: ${missing} (${(missing / cells * 100).toFixed(2)}%)`, `- Target: ${target || "not set"}`, `- Task: ${task}`, `- Features selected: ${features.length}`, "");
    }
    if (ds) {
      lines.push("## Data Validation", "", `Overall status: **${validationStatus}**`, "");
      if (validationIssues.length) { for (const i of validationIssues.slice(0, 30)) lines.push(`- [${i.severity}] ${i.category}${i.column ? ` (${i.column})` : ""}: ${i.message}`); }
      else lines.push("No validation findings.");
      lines.push("");
    }
    if (qualityScore) {
      lines.push("## Data Quality", "", `**Overall score: ${qualityScore.overall}/100**`, "", `- Completeness: ${(qualityScore.completeness * 100).toFixed(0)}%`, `- Validity: ${(qualityScore.validity * 100).toFixed(0)}%`, `- Uniqueness: ${(qualityScore.uniqueness * 100).toFixed(0)}%`, `- Consistency: ${(qualityScore.consistency * 100).toFixed(0)}%`, "", "**Recommendations:**");
      for (const r of qualityScore.recommendations) lines.push(`- ${r}`);
      lines.push("");
    }
    if (leakageWarnings.length) {
      lines.push("## Leakage Screening", "");
      for (const w of leakageWarnings) lines.push(`- [${w.risk}] ${w.column}: ${w.reason}`);
      lines.push("");
    }
    if (steps.length) {
      lines.push("## Preprocessing Pipeline", "");
      steps.forEach((s, i) => lines.push(`${i + 1}. ${s.op} — ${s.method} (${s.cols.join(", ")})`));
      lines.push("");
    }
    if (clusterResult) {
      lines.push("## Clustering", "", `Algorithm: ${clusterResult.algorithm}`, `Silhouette score: ${clusterResult.silhouette.toFixed(3)}`, "");
      for (const s of clusterResult.sizes) lines.push(`- Cluster ${s.cluster}: ${s.size} rows`);
      lines.push("");
    }
    if (result) {
      lines.push("## Model Results", "", `Algorithm: ${algo}`, `Task: ${result.metrics.task}`, "");
      if (result.metrics.task === "classification") { const m = result.metrics as ClsMetrics; lines.push(`- Accuracy: ${m.accuracy.toFixed(3)}`, `- Precision: ${m.precision.toFixed(3)}`, `- Recall: ${m.recall.toFixed(3)}`, `- F1: ${m.f1.toFixed(3)}`); }
      else { const m = result.metrics as RegMetrics; lines.push(`- R²: ${m.r2.toFixed(3)}`, `- MAE: ${m.mae.toFixed(3)}`, `- RMSE: ${m.rmse.toFixed(3)}`); }
      lines.push(`- Cross-validation mean: ${cvMean.toFixed(3)} (${result.cv.length}-fold)`, "");
    }
    return lines.join("\n");
  }
  function downloadFullReport() { downloadText(`${(projName || dsName || "ml-report").replace(/\s+/g, "-").toLowerCase()}-report.md`, buildFullReportMarkdown()); }

  // ── hyperparameter tuning: small grid search scored by k-fold CV ──
  const [tuneResults, setTuneResults] = useState<GridSearchResult[]>([]);

  const [tuning, setTuning] = useState(false);
  const [tuneMethod, setTuneMethod] = useState<"Grid Search" | "Random Search" | "Bayesian Optimization">("Grid Search");
  const [tuneTested, setTuneTested] = useState(0);
  const [modelCompareResults, setModelCompareResults] = useState<ModelCompareRow[]>([]);
  const [modelCompareRunning, setModelCompareRunning] = useState(false);
  // ── unsupervised clustering ──
  const [clusterAlgo, setClusterAlgo] = useState<"KMeans" | "DBSCAN" | "Agglomerative">("KMeans");
  const [clusterK, setClusterK] = useState(3);
  const [clusterEpsilon, setClusterEpsilon] = useState(0.5);
  const [clusterMinSamples, setClusterMinSamples] = useState(5);
  const [clusterResult, setClusterResult] = useState<ClusterResult | null>(null);
  const [clusterRunning, setClusterRunning] = useState(false);
  function runModelComparison() {
    if (!built) { setMsg("Build the model input matrix first (pick target & features)."); return; }
    setModelCompareRunning(true); setModelCompareResults([]); setMsg("");
    const candidates = Object.keys(MODELS[task]);
    const runOne = (idx: number, acc: ModelCompareRow[]) => {
      if (idx >= candidates.length) {
        setModelCompareResults([...acc].sort((a, b) => b.score - a.score));
      ...
      setTimeout(() => {
        ...
        try {
          const p: Record<string, string> = {}; MODELS[task][candidate].forEach(sp => { p[sp.name] = String(sp.def); });
          const cfg: TrainConfig = { task, algo: candidate, params: p, testSize, cvFolds };
          const scores = crossVal(cfg, built.X, built.y, built.classes?.length || 0);
          const finite = scores.filter(Number.isFinite);
          const score = finite.length ? mean(finite) : Number.NEGATIVE_INFINITY;
          acc.push({ algo: candidate, score, std: finite.length ? std(finite) : 0, ms: performance.now() - started });
        } catch (e) { acc.push({ algo: candidate, score: Number.NEGATIVE_INFINITY, std: 0, ms: performance.now() - started, error: (e as Error).message }); }
        setModelCompareResults([...acc].sort((a, b) => b.score - a.score));
        runOne(idx + 1, acc);
      }, 30);
    };
  ...
  function runClustering() {
    if (!built || !built.X.length || !built.featureNames.length) { setMsg("Select at least one feature and run preprocessing before clustering."); return; }
    setClusterRunning(true); setClusterResult(null); setMsg("");
    setTimeout(() => {
      try {
        const r = clusterAlgo === "KMeans" ? kMeans(built.X, clusterK, 100) : clusterAlgo === "DBSCAN" ? dbscan(built.X, clusterEpsilon, clusterMinSamples) : agglomerativeClustering(built.X, clusterK);
        setClusterResult(r);
        setMsg(`${r.algorithm} completed successfully: ${r.sizes.filter(x => x.cluster >= 0).length} cluster(s), silhouette ${r.silhouette.toFixed(3)}.`);
      } catch (e) { setMsg(`Clustering error: ${(e as Error).message}`); }
      setClusterRunning(false);
    }, 40);
  }
  function exportClusters() {
    if (!clusterResult || !ds) { setMsg("Run clustering first."); return; }
    const header = [...ds.columns.map(c => c.name), "cluster"];
    const rows = Array.from({length: ds.nrows}, (_,i) => [...ds.columns.map(c => c.values[i] == null ? "" : String(c.values[i])), String(clusterResult.labels[i] ?? -1)]);
    const csv = [header, ...rows].map(r => r.map(v => `"${String(v).replace(/"/g,'""')}"`).join(",")).join("\n");
    downloadBlob2(csv, "clustered-data.csv", "text/csv");
  }

  function runAutoTune() {
    if (!built) { setMsg("Build the model input matrix first (pick target & features)."); return; }
    const grid = defaultGrid(algo);
    if (!Object.keys(grid).length) { setMsg(`${MODEL_INFO[algo]?.label ?? algo} has no tunable hyperparameters to search.`); return; }
    setTuning(true); setTuneResults([]); setTuneTested(0);
    setTimeout(() => {
      try {
        const cfg: TrainConfig = { task, algo, params, testSize, cvFolds };
        const res = tuneMethod === "Grid Search" ? gridSearch(cfg, grid, built.X, built.y, built.classes?.length || 0) : tuneMethod === "Random Search" ? randomSearch(cfg, grid, built.X, built.y, built.classes?.length || 0, 20) : bayesianSearch(cfg, grid, built.X, built.y, built.classes?.length || 0, 20);
        const tagged = res.map(r => ({...r, method: tuneMethod}));
        setTuneResults(tagged); setTuneTested(tagged.length);
      } catch (e) { setMsg(`${tuneMethod} error: ` + (e as Error).message); }
      setTuning(false);
    }, 60);
  }
  function runPreprocessing() {
    if (!ds) { setMsg("Load a dataset before running preprocessing."); return; }
    if (!steps.length) { setMsg("Add at least one preprocessing step first."); return; }
    setMsg("");
    setPpPlaying(false);
    setPpIdx(0);
    try {
      const out = applyStepsSnapshots(ds, steps, 8);
      const final = out.finalColumns;
      const invalid = final
        .filter((c) => c.type === "num")
        .flatMap((c) => c.values.filter((v) => v != null && !Number.isFinite(Number(v))));
      if (invalid.length) throw new Error("Preprocessing produced non-finite numeric values. Check the selected transform/scale steps.");
      setSnaps(out.snapshots);
      setProcessedCols(final.length);
      setPpIdx(Math.min(1, Math.max(0, out.snapshots.length - 1)));
      setMsg(`Preprocessing completed successfully: ${steps.length} step(s), ${final.length} columns, ${ds.nrows} rows.`);
    } catch (e) {
      setSnaps([]);
      setProcessedCols(0);
      setMsg(`Preprocessing error: ${(e as Error).message}`);
    }
  }
  ...
  function runCrossVal() {
    ...
    setTimeout(() => {
      try {
        const cfg: TrainConfig = { task, algo, params, testSize, cvFolds };
        const strategy: "kfold" | "stratified" | "repeated" | "timeseries" = valMethod === "timeseries" ? "timeseries" : valMethod === "stratified" ? "stratified" : valMethod === "repeated" ? "repeated" : "kfold"; const groupIds = cvGroupCol ? ds?.columns.find(c=>c.name===cvGroupCol)?.values.map(v=>v==null?"__missing__":String(v)) : undefined; const res = crossValDetailed(cfg, built.X, built.y, built.classes?.length || 0, strategy, cvRepeats, groupIds);
...
${evalBlock}`;
  ...
  const MAX_BROWSER_TRAIN_ROWS = 50000;
  function capTrainingData(b: BuiltData): BuiltData {
    if (b.X.length <= MAX_BROWSER_TRAIN_ROWS) return b;
    const keep = new Set<number>();
    // Preserve at least one representative row for every class before filling evenly.
    if (b.classes?.length) for (let k = 0; k < b.classes.length; k++) {
      const i = b.y.findIndex((v) => v === k); if (i >= 0) keep.add(i);
    }
    const step = b.X.length / MAX_BROWSER_TRAIN_ROWS;
    for (let i = 0; keep.size < MAX_BROWSER_TRAIN_ROWS && i < b.X.length; i++) {
      keep.add(Math.min(b.X.length - 1, Math.floor(i * step)));
    }
    const idx = [...keep].sort((a, b) => a - b).slice(0, MAX_BROWSER_TRAIN_ROWS);
    return { ...b, X: idx.map((i) => b.X[i]), y: idx.map((i) => b.y[i]) };
  }

  function runTrain() {
    if (!ds) return;
    if (!target || !ds.columns.some((c) => c.name === target)) { setMsg("Training blocked: choose a valid target column first."); setStep("data"); return; }
    if (!features.length) { setMsg("Training blocked: select at least one feature."); setStep("model"); return; }
    if (validationStatus === "ERROR") { setMsg("Training blocked: fix the ERROR items in Data Validation first."); setStep("validate"); return; }
    if (!built || !built.X.length || !built.featureNames.length) { setMsg("Training blocked: the preprocessed feature matrix is empty."); setStep("model"); return; }
    if (built.X.some((row) => row.length !== built.featureNames.length || row.some((v) => !Number.isFinite(v)))) { setMsg("Training blocked: preprocessing produced non-finite feature values. Fix missing values/transforms and try again."); setStep("prep"); return; }
    if (task === "classification" && (built.classes?.length ?? 0) < 2) { setMsg("Training blocked: classification requires at least two target classes."); setStep("data"); return; }
    if (built.X.length < 5) { setMsg("Training blocked: at least 5 usable rows are required for a train/test split."); setStep("data"); return; }
    const uniqueTargets = new Set(built.y);
    if (task === "regression" && uniqueTargets.size < 2) { setMsg("Training blocked: regression target must contain at least two distinct values."); setStep("data"); return; }
    setMsg(""); setTraining(true); setStep("train"); setResult(null); setFlowStep(0); setTreeViz(null);
    ...
    setTimeout(() => {
      try {
        const rawBuilt = buildMatrix(ds, features, target, task, steps);
        const b = capTrainingData(rawBuilt);
        if (rawBuilt.X.length > b.X.length) setMsg(`Large dataset detected: browser training uses a deterministic ${b.X.length.toLocaleString()}-row training subset to keep the ML Lab responsive. Your original dataset remains unchanged.`);
        const nClasses = b.classes?.length || 0;
        const { Xtr, ytr, Xte, yte } = split(b.X, b.y, testSize);
        if (!Xtr.length || !Xte.length) throw new Error("Train/test split produced an empty partition. Reduce test size or add more usable rows.");
        if (task === "classification" && new Set(ytr).size < 2) throw new Error("The training partition contains only one class. Add more rows or use a different split.");
        const cfg: TrainConfig = { task, algo, params, testSize, cvFolds };
        ...
        // Model-agnostic explainability: how much held-out score drops when a feature is
        // shuffled. Works for every algorithm here (unlike coefficient/tree importance above).
        const permImp = permutationImportance(model, Xte, yte, b.featureNames, task);
        ...
        if (task === "classification") { const proba = predictProba(model, Xte); setResult({ metrics: classificationMetrics(yte, pred.map((p) => Math.round(p)), b.classes!), importance, permImp, cv, loss, ms, probaTest: { yte, proba } }); setRocClass(Math.min(1, nClasses - 1)); setThreshold(0.5); }
        else setResult({ metrics: regressionMetrics(yte, pred), importance, permImp, cv, predActual: yte.map((a, i) => [a, pred[i]] as [number, number]), loss, ms });
  ...
  function predictRow(inputs: Record<string, string>, actual?: string) {
    if (!ds || !modelRef.current || !trained) { setMsg("Train the model before making a prediction."); return; }
    ...
    let b2: BuiltData;
    try { b2 = buildMatrix(aug, features, target, task, steps); } catch (e) { setMsg(`Prediction error: ${(e as Error).message}`); return; }
    const x = b2.X[b2.X.length - 1];
    if (!x || x.length !== trained.featureNames.length || x.some((v) => !Number.isFinite(v))) { setMsg("Prediction error: input values could not be transformed into a valid numeric feature vector."); return; }
    const p = predict(modelRef.current, [x])[0];
...
${cls ? `acc = sum(1 for i in te if predict(X[i]) == y[i]) / len(te); print("accuracy:", round(acc, 3))` : `mte = sum(y[i] for i in te)/len(te); ss=sum((y[i]-predict(X[i]))**2 for i in te); st=sum((y[i]-mte)**2 for i in te); print("R2:", round(1-ss/st,3))`}`;
  ...
  function ReportCard({ title, icon, children, exportText }: { title: string; icon: string; children: React.ReactNode; exportText?: string }) {
    return <div className="card" style={{ marginTop: 16, borderLeft: "3px solid var(--accent)" }}>
      <div className="card-h"><span className="t">{icon} {title}</span><div className="row" style={{ gap: 8 }}>{exportText && <button className="btn ghost sm" onClick={() => downloadText(`${title.replace(/\s+/g, "-").toLowerCase()}.md`, exportText)}>⬇ Export</button>}<span className="mono r">current dataset</span></div></div>
      <div className="card-b">{children}</div>
    </div>;
  }

  const dataReport = ds ? (() => {
    const numeric = ds.columns.filter(c => c.type === "num");
    const categorical = ds.columns.filter(c => c.type === "cat");
    const missing = ds.columns.reduce((n,c) => n + c.values.filter(v => v == null || v === "").length, 0);
    const cells = Math.max(1, ds.nrows * ds.columns.length);
    const keys: string[] = [];
    for (let i = 0; i < ds.nrows; i++) keys.push(ds.columns.map(c => String(c.values[i] ?? "")).join("\u001f"));
    const duplicateRows = ds.nrows - new Set(keys).size;
    return { numeric: numeric.length, categorical: categorical.length, missing, missingPct: missing / cells * 100, duplicateRows, duplicatePct: duplicateRows / Math.max(1, ds.nrows) * 100 };
  })() : null;
  ...
  return (
    <>
      <div className="lab-head">
        ...
        <div className="acts"><button className="btn ghost sm" onClick={saveProject}>{savedMsg || "💾 Save"}</button><button className="btn ghost sm" onClick={downloadFullReport} disabled={!ds}>📥 Download report</button><button className="btn ghost sm" onClick={() => setShowCode(true)}>&lt;/&gt; Get code</button></div>
      </div>
      ...
      <div className="stepper">
        <button className={step === "problem" ? "on" : ""} onClick={() => setStep("problem")}><b>1</b>Problem</button>
        <button className={step === "data" ? "on" : ""} onClick={() => setStep("data")}><b>2</b>Data</button>
        {stepBtn("validate", 3, "Validate")}
        {stepBtn("eda", 4, "EDA")}
        {stepBtn("quality", 5, "Quality")}
        {stepBtn("leakage", 6, "Leakage")}
        {stepBtn("prep", 7, "Preprocessing")}
        {stepBtn("model", 8, "Model")}
        {stepBtn("validation", 9, "Validation")}
        {stepBtn("train", 10, "Train")}
        <button className={step === "deploy" ? "on" : ""} disabled={!result} onClick={() => setStep("deploy")}><b>11</b>Test &amp; Export</button>
      </div>
      <div className="row" style={{ justifyContent: "flex-end", marginTop: 10 }}>
        <LearnButton topic={step} />
      </div>
      ...
      {step === "problem" && (
        <div className="card">
          <div className="card-h"><span className="t">Problem definition</span><LearnButton topic="problem" compact /></div>
          <div className="card-b">
            <div className="teach-note"><span className="ic">🧭</span><span>Define the problem before touching data — it drives which metric matters, what a “good” model looks like, and what should never leak into training.</span></div>
            <div className="row" style={{ gap: 14, flexWrap: "wrap", marginTop: 12 }}>
              <div style={{ flex: "1 1 260px" }}><label className="fld">Project name</label><input type="text" value={projName} onChange={(e) => setProjName(e.target.value)} placeholder="e.g. Customer Churn Predictor" /></div>
              <div style={{ flex: "1 1 260px" }}>
                <label className="fld">Problem type</label>
                <div className="rangebtns">
                  {(["classification", "regression", "clustering", "timeseries"] as ProblemType[]).map((pt) => (
                    <button key={pt} className={problemType === pt ? "on" : ""} onClick={() => { setProblemType(pt); if (pt === "clustering" && ds) { setFeatures(ds.columns.map(c => c.name)); setClusterResult(null); } }}>{pt === "timeseries" ? "Time Series" : pt.charAt(0).toUpperCase() + pt.slice(1)}</button>
                  ))}
                </div>
                {ds && (problemType === "classification" || problemType === "regression") && problemType !== task && (
                  <div className="note" style={{ marginTop: 6 }}>Target column “{target}” looks like <b>{task}</b> from the data. <button className="btn ghost sm" style={{ padding: "2px 8px" }} onClick={() => setProblemType(task)}>Use {task}</button></div>
                )}
              </div>
            </div>
            <label className="fld" style={{ marginTop: 14 }}>Problem statement</label>
            <textarea rows={2} value={problemStatement} onChange={(e) => setProblemStatement(e.target.value)} placeholder="What real-world problem are we solving?" />
            <div className="row" style={{ gap: 14, flexWrap: "wrap", marginTop: 12 }}>
              <div style={{ flex: "1 1 300px" }}><label className="fld">Business objective</label><textarea rows={2} value={bizObjective} onChange={(e) => setBizObjective(e.target.value)} placeholder="e.g. Reduce customer churn by 10% this quarter" /></div>
              <div style={{ flex: "1 1 300px" }}><label className="fld">ML objective</label><textarea rows={2} value={mlObjective} onChange={(e) => setMlObjective(e.target.value)} placeholder="e.g. Predict which customers will churn in the next 30 days" /></div>
            </div>
            <div className="row" style={{ gap: 14, flexWrap: "wrap", marginTop: 12 }}>
              <div style={{ flex: "1 1 220px" }}><label className="fld">Target column</label><input type="text" value={target} readOnly placeholder="pick in Data step" style={{ opacity: 0.8 }} /><span className="note">Set on the Data step — shown here for reference.</span></div>
              <div style={{ flex: "1 1 220px" }}><label className="fld">Primary evaluation metric</label><input type="text" value={primaryMetric} onChange={(e) => setPrimaryMetric(e.target.value)} placeholder={problemType === "classification" ? "e.g. F1 score" : "e.g. RMSE"} /></div>
              <div style={{ flex: "1 1 220px" }}><label className="fld">Secondary metrics</label><input type="text" value={secondaryMetrics} onChange={(e) => setSecondaryMetrics(e.target.value)} placeholder="e.g. Precision, Recall" /></div>
            </div>
            <div className="row" style={{ gap: 14, flexWrap: "wrap", marginTop: 12 }}>
              <div style={{ flex: "1 1 300px" }}><label className="fld">Expected output</label><textarea rows={2} value={expectedOutput} onChange={(e) => setExpectedOutput(e.target.value)} placeholder="e.g. A risk score 0-1 per customer, refreshed daily" /></div>
              <div style={{ flex: "1 1 300px" }}><label className="fld">Constraints</label><textarea rows={2} value={problemConstraints} onChange={(e) => setProblemConstraints(e.target.value)} placeholder="e.g. Must run in <100ms, no PII in features, interpretable model required" /></div>
            </div>
            {(projName || problemStatement || bizObjective || mlObjective) && (
              <div className="teach-note" style={{ marginTop: 14, alignItems: "flex-start" }}>
                <span className="ic">📋</span>
                <span>
                  <b>{projName || "Untitled project"}</b> — {problemType === "timeseries" ? "Time Series" : problemType.charAt(0).toUpperCase() + problemType.slice(1)}
                  {target && <> · target <b>{target}</b></>}{primaryMetric && <> · optimizing <b>{primaryMetric}</b></>}
                  {problemStatement && <div style={{ marginTop: 4 }}>{problemStatement}</div>}
                </span>
              </div>
            )}
            <div className="stepnav"><button className="btn" onClick={() => setStep("data")}>Next: Data →</button></div>
          </div>
        </div>
      )}
      ...
      {step === "data" && (
        <div className="card">
          <div className="card-h"><span className="t">Connect your data</span><LearnButton topic="data" compact /><div className="tabs"><button className={dataTab === "sample" ? "on" : ""} onClick={() => setDataTab("sample")}>Sample</button><button className={dataTab === "upload" ? "on" : ""} onClick={() => setDataTab("upload")}>Upload CSV</button><button className={dataTab === "db" ? "on" : ""} onClick={() => setDataTab("db")}>Database</button></div></div>
          <div className="card-b">
            ...
            {dataTab === "upload" && (<div>
              <div className="dropzone" onClick={() => fileRef.current?.click()}>Click to upload a <b>CSV</b> or <b>CSV.GZ</b> file (parsed &amp; trained locally)</div>
              <input ref={fileRef} type="file" accept=".csv,.tsv,.csv.gz,.gz,.gzip,text/csv,application/gzip" onChange={onFile} style={{ display: "none" }} />
              <div style={{ marginTop: 12, padding: 12, border: "1px solid var(--border)", borderRadius: 10, background: "var(--panel-2)" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
                  <div>
                    <b>🗜️ Dataset Compressor</b>
                    <div className="note" style={{ marginTop: 4 }}>Compress a large CSV/TSV into <code>.csv.gz</code> before uploading. This can help when your upload limit is 50 MB.</div>
                  </div>
                  <label className="btn sm" style={{ cursor: "pointer" }}>Compress dataset<input type="file" accept=".csv,.tsv,text/csv" onChange={(e) => { const f = e.target.files?.[0]; if (f) void compressDatasetFile(f); e.target.value = ""; }} style={{ display: "none" }} /></label>
                </div>
                <div className="note" style={{ marginTop: 8 }}>The compressor now <b>compresses, verifies, and loads the compressed dataset automatically</b>. You do not need to select the original 123 MB CSV again. A <code>.csv.gz</code> copy is also downloaded for external upload/storage.</div>
                <div className="note" style={{ marginTop: 6, color: "var(--faint)" }}>⚠️ GZIP is lossless: it cannot guarantee a file will become ≤50 MB. If the actual <code>.gz</code> result is still above 50 MB, no rows/columns are removed and the dataset is not altered.</div>
              </div>
            </div>)}
            ...
            {ds && (<>
              {dataReport && <ReportCard title="Data Upload Report" icon="📁" exportText={`# Data Upload Report\n\n- Rows: ${ds.nrows}\n- Columns: ${ds.columns.length}\n- Numerical: ${dataReport.numeric}\n- Categorical: ${dataReport.categorical}\n- Missing cells: ${dataReport.missing} (${dataReport.missingPct.toFixed(2)}%)\n- Duplicate rows: ${dataReport.duplicateRows} (${dataReport.duplicatePct.toFixed(2)}%)\n- Target: ${target || "not selected"}\n- Task: ${task}\n- Features selected: ${features.length}\n`}>
                <div className="metric-grid" style={{ marginBottom: 12 }}><div className="metric"><span className="v">{ds.nrows.toLocaleString()}</span><span className="k">Rows</span></div><div className="metric"><span className="v">{ds.columns.length}</span><span className="k">Columns</span></div><div className="metric"><span className="v">{dataReport.numeric}</span><span className="k">Numerical</span></div><div className="metric"><span className="v">{dataReport.categorical}</span><span className="k">Categorical</span></div></div>
                <div className="split col-2e"><div><b>Data quality snapshot</b><div className="note" style={{marginTop:6}}>Missing cells: {dataReport.missing.toLocaleString()} ({dataReport.missingPct.toFixed(2)}%) · Duplicate rows: {dataReport.duplicateRows.toLocaleString()} ({dataReport.duplicatePct.toFixed(2)}%).</div></div><div><b>ML readiness</b><div className="note" style={{marginTop:6}}>{dataReport.missingPct === 0 && dataReport.duplicateRows === 0 ? "✓ Clean starting point" : "⚠ Review missing values and duplicates before training."}</div></div></div>
                <div className="note" style={{marginTop:10}}>Target: <b>{target || "not selected"}</b> · Task: <b>{task}</b> · Features selected: <b>{features.length}</b>.</div>
              </ReportCard>}
              ...
              <div className="card" style={{ marginBottom: 14, borderLeft: "3px solid var(--accent)" }}>
                <div className="card-h"><span className="t">🪄 Auto pipeline detection</span><span className="mono r">one click</span></div>
                <div className="card-b">
                  <div className="note" style={{ marginBottom: 10 }}>Scores every column as a target candidate, infers the task, drops obvious ID-like/constant columns, and drafts a full preprocessing pipeline (impute → encode → scale). Shown as a preview — nothing changes until you apply it.</div>
                  <button className="btn ghost sm" onClick={runAutoDetect}>Analyze dataset</button>
                  {autoPlan && (
                    <div style={{ marginTop: 12 }}>
                      <div className="row" style={{ gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
                        <span className="pill">target <b style={{ marginLeft: 4 }}>{autoPlan.target}</b></span>
                        <span className="pill">task <b style={{ marginLeft: 4 }}>{autoPlan.task}</b></span>
                        <span className="pill">{autoPlan.features.length} features</span>
                        <span className="pill">{autoPlan.steps.length} preprocessing steps</span>
                      </div>
                      <label className="fld">Target candidates considered</label>
                      <div className="note" style={{ marginBottom: 8 }}>{autoPlan.targetCandidates.map((c) => `${c.name} (score ${c.score.toFixed(1)})`).join(" · ")}</div>
                      <label className="fld">Draft pipeline</label>
                      <div className="note" style={{ marginBottom: 8 }}>{autoPlan.steps.map((s, i) => <div key={i}>{i + 1}. {s.op} — {s.method} ({s.cols.length} column(s))</div>)}</div>
                      {autoPlan.warnings.length > 0 && (<><label className="fld">Warnings</label><div className="note" style={{ marginBottom: 8 }}>{autoPlan.warnings.map((w, i) => <div key={i}>⚠ {w}</div>)}</div></>)}
                      <div className="row" style={{ gap: 10 }}>
                        <button className="btn" onClick={applyAutoPlan}>✓ Apply this pipeline</button>
                        <button className="btn ghost" onClick={() => setAutoPlan(null)}>Dismiss</button>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="stepnav"><button className="btn ghost" onClick={() => setStep("problem")}>← Problem</button><button className="btn" onClick={() => setStep("validate")}>Next: Data Validation →</button></div>
            </>)}
          </div>
        </div>
      )}

      {/* STEP 1b DATA VALIDATION (§3) */}
      {step === "validate" && ds && (() => {
        const bySeverity = (sev: Severity) => validationIssues.filter((i) => i.severity === sev);
        const errs = bySeverity("ERROR"), warns = bySeverity("WARNING");
        const statusColor = validationStatus === "ERROR" ? "var(--crit)" : validationStatus === "WARNING" ? "#f59e0b" : "#3ecf7f";
        const statusIcon = validationStatus === "ERROR" ? "🔴" : validationStatus === "WARNING" ? "🟠" : "🟢";
        const catLabel: Record<string, string> = { Schema: "Schema", Type: "Data type", Constraint: "Constraint", Value: "Value", Duplicate: "Duplicate" };
        return (
          <div className="card">
            <div className="card-h">
              <span className="t">Data validation</span><LearnButton topic="validate" compact />
              <div className="row" style={{ gap: 8 }}>
                {refSchema ? (<><span className="note">reference saved {new Date(refSchema.savedAt).toLocaleTimeString()}</span><button className="btn ghost sm" onClick={() => setRefSchema(null)}>Clear reference</button></>)
                  : <button className="btn ghost sm" onClick={() => setRefSchema(snapshotSchema(ds))}>📌 Save schema as reference</button>}
              </div>
            </div>
            <div className="card-b">
              <div className="teach-note"><span className="ic">🧪</span><span>Runs schema, type, constraint, value, and duplicate checks against the loaded dataset. Save the current schema as a <b>reference</b> to also catch schema drift (missing/new/renamed/retyped columns) on future loads.</span></div>
              <div className="row" style={{ gap: 10, alignItems: "center", margin: "14px 0" }}>
                <span className="pill" style={{ fontSize: 14 }}>{statusIcon} <b style={{ color: statusColor, marginLeft: 4 }}>{validationStatus}</b></span>
                <span className="pill">{errs.length} error{errs.length === 1 ? "" : "s"}</span>
                <span className="pill">{warns.length} warning{warns.length === 1 ? "" : "s"}</span>
              </div>
              <div style={{ overflowX: "auto" }}>
                <table className="dtable">
                  <tbody>
                    <tr><th>Severity</th><th>Category</th><th>Column</th><th>Reason</th></tr>
                    {validationIssues.map((i) => (
                      <tr key={i.id}>
                        <td style={{ color: i.severity === "ERROR" ? "var(--crit)" : i.severity === "WARNING" ? "#f59e0b" : "#3ecf7f" }}>
                          {i.severity === "ERROR" ? "🔴" : i.severity === "WARNING" ? "🟠" : "🟢"} {i.severity}
                        </td>
                        <td>{catLabel[i.category]}</td>
                        <td>{i.column ?? <span className="note">—</span>}</td>
                        <td>{i.message}</td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <ReportCard title="Data Validation Report" icon="🛡️" exportText={`# Data Validation Report\n\nOverall status: ${validationStatus}\nChecks / findings: ${validationIssues.length} (${errs.length} errors, ${warns.length} warnings)\n\n${validationIssues.map((i) => `- [${i.severity}] ${i.category}${i.column ? ` (${i.column})` : ""}: ${i.message}`).join("\n") || "No validation findings."}\n`}>
                <div className="metric-grid" style={{marginBottom:12}}><div className="metric"><span className="v">{validationIssues.length}</span><span className="k">Checks / findings</span></div><div className="metric"><span className="v">{errs.length}</span><span className="k">Errors</span></div><div className="metric"><span className="v">{warns.length}</span><span className="k">Warnings</span></div><div className="metric"><span className="v">{validationStatus}</span><span className="k">Overall status</span></div></div>
                <div className="note">Schema, data type, constraint, value and duplicate checks are evaluated for the current dataset. {refSchema ? "A saved reference schema is also being compared for drift." : "Save a reference schema to detect future schema drift."}</div>
                <div style={{marginTop:10}}>{validationIssues.length ? validationIssues.slice(0,8).map(i => <div key={i.id} className="row" style={{justifyContent:"space-between",padding:"4px 0"}}><span>{i.severity === "ERROR" ? "🔴" : i.severity === "WARNING" ? "🟠" : "🟢"} {i.message}</span><span className="mono">{i.column ?? "dataset"}</span></div>) : <div className="note">✓ No validation findings.</div>}</div>
              </ReportCard>
              <div className="stepnav"><button className="btn ghost" onClick={() => setStep("data")}>← Data</button><button className="btn" onClick={() => setStep("eda")}>Next: EDA →</button></div>
            </div>
          </div>
        );
      })()}
        ...
        return (
          <div className="split" style={{ gridTemplateColumns: "244px 1fr" }}>
            ...
            <div>
              ...
              <div className="stepnav"><button className="btn ghost" onClick={() => setStep("validate")}>← Validate</button><button className="btn" onClick={() => setStep("quality")}>Next: Data Quality →</button></div>
            </div>
          </div>
        );
      })()}

      {/* STEP 2b DATA QUALITY ASSESSMENT (§4) */}
      {step === "quality" && ds && qualityScore && (() => {
        const q = qualityScore;
        const pct = (x: number) => `${(x * 100).toFixed(0)}%`;
        const scoreColor = q.overall >= 85 ? "#3ecf7f" : q.overall >= 60 ? "#f59e0b" : "var(--crit)";
        const dims: [string, number][] = [["Completeness", q.completeness], ["Validity", q.validity], ["Uniqueness", q.uniqueness], ["Consistency", q.consistency]];
        return (
          <div className="card">
            <div className="card-h"><span className="t">Data quality dashboard</span></div>
            <div className="card-b">
              <div className="row" style={{ gap: 24, alignItems: "center", flexWrap: "wrap", marginBottom: 16 }}>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 40, fontWeight: 700, color: scoreColor, lineHeight: 1 }}>{q.overall}</div>
                  <div className="note">Data Quality Score / 100</div>
                </div>
                <div style={{ flex: 1, minWidth: 260, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  {dims.map(([label, val]) => (
                    <div key={label}><div className="note" style={{ marginBottom: 3 }}>{label}: {pct(val)}</div>
                      <div style={{ height: 8, borderRadius: 4, background: "var(--panel)", border: "1px solid var(--border)", overflow: "hidden" }}>
                        <div style={{ width: pct(val), height: "100%", background: val > 0.85 ? "#3ecf7f" : val > 0.6 ? "#f59e0b" : "var(--crit)" }} />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="row" style={{ gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
                <span className="pill">missing {pct(q.missingPct)}</span>
                <span className="pill">duplicates {pct(q.duplicatePct)}</span>
                <span className="pill">invalid values {pct(q.invalidPct)}</span>
                <span className="pill">outliers (IQR) {pct(q.outlierPct)}</span>
              </div>
              {q.flags.length > 0 && (<>
                <label className="fld">Column flags</label>
                <div className="chips" style={{ marginBottom: 14 }}>
                  {q.flags.map((f, i) => (<span key={i} className="chip" style={{ cursor: "default" }}><b>{f.column}</b> · {f.kind} — {f.detail}</span>))}
                </div>
              </>)}
              <label className="fld">Recommendations</label>
              <ul style={{ margin: "6px 0 0", paddingLeft: 20 }}>{q.recommendations.map((r, i) => <li key={i} className="note" style={{ marginBottom: 4 }}>{r}</li>)}</ul>
              <div className="stepnav"><button className="btn ghost" onClick={() => setStep("eda")}>← EDA</button><button className="btn" onClick={() => setStep("leakage")}>Next: Leakage Detection →</button></div>
            </div>
          </div>
        );
      })()}

      {/* STEP 2c LEAKAGE DETECTION (§7) */}
      {step === "leakage" && ds && (() => {
        const riskColor = (r: LeakageWarning["risk"]) => (r === "HIGH" ? "var(--crit)" : r === "MEDIUM" ? "#f59e0b" : "var(--faint)");
        const sorted = [...leakageWarnings].sort((a, b) => ({ HIGH: 0, MEDIUM: 1, LOW: 2 }[a.risk] - { HIGH: 0, MEDIUM: 1, LOW: 2 }[b.risk]));
        return (
          <div className="card">
            <div className="card-h"><span className="t">Leakage detection</span><LearnButton topic="leakage" compact /></div>
            <div className="card-b">
              <div className="teach-note"><span className="ic">🕳️</span><span>Flags features that name-match the target with an outcome-ish term, correlate near-perfectly with it, or (for categorical/binned numeric features) predict it almost deterministically. High/medium risk features should usually be excluded before training.</span></div>
              {sorted.length === 0 && <div className="teach-note" style={{ marginTop: 12 }}><span className="ic">✅</span><span>No suspicious features detected against target “{target}”.</span></div>}
              {sorted.length > 0 && (
                <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 10 }}>
                  {sorted.map((w) => {
                    const excluded = leakageExcluded.has(w.column) || !features.includes(w.column);
                    return (
                      <div key={w.column} className="teach-note" style={{ alignItems: "flex-start", borderColor: riskColor(w.risk) }}>
                        <span className="ic">{w.risk === "HIGH" ? "🚨" : w.risk === "MEDIUM" ? "⚠️" : "ℹ️"}</span>
                        <span style={{ flex: 1 }}>
                          <b>Potential Data Leakage Detected</b><br />
                          Feature: <b>{w.column}</b> &nbsp; Risk: <b style={{ color: riskColor(w.risk) }}>{w.risk}</b>
                          <div style={{ marginTop: 4 }}>{w.reason}</div>
                        </span>
                        {excluded
                          ? <button className="btn ghost sm" onClick={() => reincludeFeature(w.column)}>Re-include</button>
                          : <button className="btn sm" onClick={() => excludeLeakyFeature(w.column)}>Exclude from features</button>}
                      </div>
                    );
                  })}
                </div>
              )}
              {leakageExcluded.size > 0 && <div className="note" style={{ marginTop: 12 }}>Excluded from features: {[...leakageExcluded].join(", ")}</div>}
              <div className="stepnav"><button className="btn ghost" onClick={() => setStep("quality")}>← Quality</button><button className="btn" onClick={() => setStep("prep")}>Next: Preprocess →</button></div>
            </div>
          </div>
        );
      })()}
      ...
      {step === "prep" && ds && (
        <>
          <div className="split col-2e">
            <div className="card"><div className="card-h"><span className="t">Add a preprocessing step</span><LearnButton topic="prep" compact /></div>
              <div className="card-b">
          ...
          <ReportCard title="Preprocessing Report" icon="🧹" exportText={`# Preprocessing Report\n\nPipeline steps: ${steps.length}\nFinal columns: ${snaps.length ? processedCols : "—"}\nExecution: ${snaps.length ? "PASS" : "NOT RUN"}\nRows retained: ${ds.nrows}\n\n${steps.map((st, i) => `${i + 1}. ${st.op} — ${st.method} (${st.cols.join(", ")})`).join("\n") || "No steps added."}\n`}>
            <div className="metric-grid" style={{marginBottom:12}}><div className="metric"><span className="v">{steps.length}</span><span className="k">Pipeline steps</span></div><div className="metric"><span className="v">{snaps.length ? processedCols : "—"}</span><span className="k">Final columns</span></div><div className="metric"><span className="v">{snaps.length ? "PASS" : "NOT RUN"}</span><span className="k">Execution</span></div><div className="metric"><span className="v">{ds.nrows}</span><span className="k">Rows retained</span></div></div>
            {steps.length ? <div>{steps.map((st,i)=><div key={i} className="row" style={{justifyContent:"space-between",padding:"4px 0"}}><span><b>{i+1}.</b> {st.op}</span><span className="mono">{st.method} · {st.cols.length} column(s)</span></div>)}</div> : <div className="note">Add preprocessing steps to build the report.</div>}
            {snaps.length > 1 && <div className="teach-note" style={{marginTop:10}}><span className="ic">✓</span><span>Preprocessing completed. The final matrix contains {processedCols} columns and is ready for model input.</span></div>}
          </ReportCard>
          <div className="card" style={{ marginTop: 16 }}>
            <div className="card-h"><span className="t">Feature engineering</span><span className="mono r">create a derived column</span></div>
            <div className="card-b">
              <div className="split col-2e">
                <div>
                  <label className="fld">Operation</label>
                  <select value={engOp} onChange={(e) => { setEngOp(e.target.value as EngineerOp); setEngColB(""); }}>{ENGINEER_OPS.map((o) => <option key={o}>{o}</option>)}</select>
                  <label className="fld" style={{ marginTop: 12 }}>Column A</label>
                  <select value={engColA} onChange={(e) => setEngColA(e.target.value)}><option value="">— pick —</option>{numEngCols.map((c) => <option key={c}>{c}</option>)}</select>
                  {ENGINEER_NEEDS_B[engOp] && (<>
                    <label className="fld" style={{ marginTop: 12 }}>Column B</label>
                    <select value={engColB} onChange={(e) => setEngColB(e.target.value)}><option value="">— pick —</option>{numEngCols.filter((c) => c !== engColA).map((c) => <option key={c}>{c}</option>)}</select>
                  </>)}
                  <button className="btn" style={{ marginTop: 14 }} onClick={addEngineeredFeature} disabled={!engColA || (ENGINEER_NEEDS_B[engOp] && !engColB)}>+ Create feature</button>
                </div>
                <div className="note">Builds a new numeric column from existing ones — squares, ratios, sums, interactions — a common way to surface non-linear relationships the raw columns don&apos;t expose directly. The new column is appended to the dataset, auto-selected as a feature, and given default impute + scale steps.</div>
              </div>
            </div>
          </div>
          <div className="card" style={{ marginTop: 16 }}>
            <div className="card-h"><span className="t">🤖 AI-suggested feature engineering</span><span className="mono r">rule-based, not an LLM call</span></div>
            <div className="card-b">
              <div className="teach-note"><span className="ic">🧠</span><span>Scans your numeric columns for skew, weak-but-improvable correlations, and pairs that combine into a stronger signal than either column alone — then proposes the exact engineered feature, with the reasoning and a risk label, for you to confirm.</span></div>
              <div className="row" style={{ marginTop: 12 }}><button className="btn ghost sm" disabled={suggestBusy || !target} onClick={runFeatureSuggestions}>{suggestBusy ? "Scanning…" : "✨ Suggest features"}</button>{!target && <span className="note">pick a target column first</span>}</div>
              {featureSuggestions && (featureSuggestions.length === 0 ? (
                <div className="note" style={{ marginTop: 10 }}>No engineered feature clearly beat its source columns — the raw features already carry the signal.</div>
              ) : (
                <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 10 }}>
                  {featureSuggestions.map((s) => (
                    <div key={s.id} className="teach-note" style={{ alignItems: "flex-start" }}>
                      <span className="ic">{s.risk === "High" ? "🚨" : s.risk === "Medium" ? "⚠️" : "💡"}</span>
                      <span style={{ flex: 1 }}>
                        <b>{s.name}</b> <span className="note">({s.op})</span> — risk: <b>{s.risk}</b>
                        <div style={{ marginTop: 4 }}>{s.reason}</div>
                        <div className="note" style={{ marginTop: 2 }}>Expected benefit: {s.benefit}</div>
                      </span>
                      <button className="btn sm" onClick={() => applyFeatureSuggestion(s)}>+ Add feature</button>
                    </div>
                  ))}
                </div>
              ))}
      ...
      {step === "model" && ds && (
        <>
          {problemType === "clustering" ? (
            <div className="card">
              <div className="card-h"><span className="t">🧩 Clustering Lab</span><LearnButton topic="model" compact /><span className="mono r">unsupervised learning</span></div>
              <div className="card-b">
                <div className="teach-note"><span className="ic">🧠</span><span>Clustering groups similar rows without a target column. The same preprocessed numeric matrix is used for every algorithm, so the result is reproducible and exportable.</span></div>
                <div className="row" style={{gap:14,flexWrap:"wrap",marginTop:14}}>
                  <div><label className="fld">Algorithm</label><select value={clusterAlgo} onChange={e=>{const value=e.target.value; if(value === "KMeans" || value === "DBSCAN" || value === "Agglomerative") setClusterAlgo(value); setClusterResult(null)}}><option>KMeans</option><option>DBSCAN</option><option>Agglomerative</option></select></div>
                  {clusterAlgo !== "DBSCAN" && <div><label className="fld">Number of clusters (k)</label><input type="number" min={2} max={Math.max(2, Math.min(20, ds.nrows))} value={clusterK} onChange={e=>setClusterK(Math.max(2,Number(e.target.value)||2))} style={{width:90}} /></div>}
                  {clusterAlgo === "DBSCAN" && <><div><label className="fld">Epsilon</label><input type="number" min={0.01} step={0.05} value={clusterEpsilon} onChange={e=>setClusterEpsilon(Math.max(0.01,Number(e.target.value)||0.5))} style={{width:90}} /></div><div><label className="fld">Min samples</label><input type="number" min={2} max={100} value={clusterMinSamples} onChange={e=>setClusterMinSamples(Math.max(2,Number(e.target.value)||5))} style={{width:90}} /></div></>}
                  <button className="btn" style={{marginTop:18}} onClick={runClustering} disabled={clusterRunning || !built}>{clusterRunning ? "Clustering…" : "▶ Run clustering"}</button>
                </div>
                <label className="fld" style={{marginTop:14}}>Features ({features.length})</label>
                <div className="checklist">{ds.columns.map(c=><span key={c.name} className={`chk ${features.includes(c.name)?"on":""}`} onClick={()=>setFeatures(f=>f.includes(c.name)?f.filter(x=>x!==c.name):[...f,c.name])}>{c.name}</span>)}</div>
                {clusterResult && <ReportCard title="Clustering Report" icon="🧩" exportText={`# Clustering Report\n\nAlgorithm: ${clusterResult.algorithm}\nSilhouette score: ${clusterResult.silhouette.toFixed(3)}\nClusters: ${clusterResult.sizes.filter((x) => x.cluster >= 0).length}\nNoise points: ${clusterResult.labels.filter((x) => x < 0).length}\n\n${clusterResult.sizes.map((x) => `- Cluster ${x.cluster < 0 ? "Noise" : x.cluster}: ${x.size} rows (${(x.size / ds.nrows * 100).toFixed(1)}%)`).join("\n")}\n`}>
                  <div className="metric-grid" style={{marginBottom:12}}><div className="metric"><span className="v">{clusterResult.algorithm}</span><span className="k">Algorithm</span></div><div className="metric"><span className="v">{clusterResult.silhouette.toFixed(3)}</span><span className="k">Silhouette score</span></div><div className="metric"><span className="v">{clusterResult.sizes.filter(x=>x.cluster>=0).length}</span><span className="k">Clusters</span></div><div className="metric"><span className="v">{clusterResult.labels.filter(x=>x<0).length}</span><span className="k">Noise points</span></div></div>
                  <div style={{overflowX:"auto"}}><table className="dtable"><tbody><tr><th>Cluster</th><th>Rows</th><th>Share</th></tr>{clusterResult.sizes.map(x=><tr key={x.cluster}><td>{x.cluster<0?"Noise":x.cluster}</td><td>{x.size}</td><td>{(x.size/ds.nrows*100).toFixed(1)}%</td></tr>)}</tbody></table></div>
                  <div className="teach-note" style={{marginTop:10}}><span className="ic">💡</span><span><b>Interpretation:</b> silhouette near 1 means compact, well-separated groups; near 0 means overlapping groups; negative values suggest points may be assigned to the wrong cluster.</span></div>
                  <button className="btn" style={{marginTop:10}} onClick={exportClusters}>⬇ Export clustered CSV</button>
                </ReportCard>}
                <div className="stepnav"><button className="btn ghost" onClick={()=>setStep("eda")}>← Back to EDA</button></div>
              </div>
            </div>
          ) : null}
          {problemType !== "clustering" && <>
          <div className="card">
            <div className="card-h"><span className="t">Choose a model</span><LearnButton topic="model" compact /><span className="mono r">{Object.keys(MODELS[task]).length} available · {task}</span></div>
            <div className="card-b">
              ...
              <div className="model-grid">
                {Object.keys(MODELS[task]).map((a) => { const info = MODEL_INFO[a]; const np = MODELS[task][a].length; return (
                  <div key={a} className={`model-card ${algo === a ? "on" : ""}`} onClick={() => { setAlgo(a); setParamsFor(task, a); }}>
                    ...
                    <div className="mc-foot">{np ? `${np} hyperparameter${np > 1 ? "s" : ""}` : "no hyperparameters"}{algo === a ? " · selected" : ""}<button className="btn ghost sm" style={{ marginLeft: 8 }} onClick={(e) => { e.stopPropagation(); setLearnTopic("model"); }}>📚 Learn</button></div>
                  </div>); })}
              </div>
            </div>
          </div>
            ...
            <div className="card"><div className="card-h"><span className="t">Target &amp; features</span></div>
              <div className="card-b">
                ...
                <div style={{ marginTop: 14, borderTop: "1px solid var(--border)", paddingTop: 12 }}>
                  <label className="fld">Feature selection</label>
                  <div className="row"><button className="btn" onClick={runFeatureSelection}>Rank by correlation with target</button></div>
                  {fsScores && (<>
                    <div className="note" style={{ margin: "8px 0" }}>|Pearson correlation| with target, highest first — one-hot encoded columns are rolled back up to their source column.</div>
                    <div style={{ maxHeight: 180, overflowY: "auto" }}>
                      {fsScores.map((s) => <div key={s.name} className="row" style={{ justifyContent: "space-between", padding: "3px 0" }}><span className="mono">{s.name}</span><span className="note">{s.score.toFixed(3)}</span></div>)}
                    </div>
                    <div className="row" style={{ marginTop: 10, gap: 8 }}>
                      <span className="note">Keep top</span>
                      <input type="text" value={fsTopN} onChange={(e) => setFsTopN(Number(e.target.value) || 1)} style={{ width: 50 }} />
                      <button className="btn" onClick={applyTopFeatures}>Apply</button>
                    </div>
                  </>)}
                </div>
              </div>
            </div>
            <div className="card"><div className="card-h"><span className="t">Hyperparameters</span><LearnButton topic="hyperparameters" compact /><span className="mono r">{MODEL_INFO[algo]?.label ?? algo}</span></div>
              <div className="card-b"><div className="hyper">{MODELS[task][algo].length === 0 && <div className="note">This model has no tunable hyperparameters.</div>}{MODELS[task][algo].map((sp) => (
                ...
                {Object.keys(defaultGrid(algo)).length > 0 && (
                  <div style={{ marginTop: 14, borderTop: "1px solid var(--border)", paddingTop: 12 }}>
                    <div className="row" style={{gap:8,flexWrap:"wrap"}}><select value={tuneMethod} onChange={(e)=>{const value=e.target.value; if(value === "Grid Search" || value === "Random Search" || value === "Bayesian Optimization") setTuneMethod(value)}}><option>Grid Search</option><option>Random Search</option><option>Bayesian Optimization</option></select><button className="btn" onClick={runAutoTune} disabled={tuning}>{tuning ? "Searching…" : `🔍 Run ${tuneMethod}`}</button></div>
                    <span className="note" style={{ marginLeft: 8 }}>Cross-validation score drives the search; bounded for browser performance.</span>
                    {tuneResults.length > 0 && (() => { const best=tuneResults[0]; return <div style={{marginTop:10}}>
                      <div className="metric-grid" style={{marginBottom:10}}><div className="metric"><span className="v">{Object.entries(best.params).map(([k,v])=>`${k}=${v}`).join(", ")}</span><span className="k">Best parameters</span></div><div className="metric"><span className="v">{best.meanScore.toFixed(4)}</span><span className="k">Best CV score</span></div><div className="metric"><span className="v">{tuneTested}</span><span className="k">Parameter combinations tested</span></div><div className="metric"><span className="v">{best.method ?? tuneMethod}</span><span className="k">Optimization result</span></div></div>
                      {tuneResults.slice(0,5).map((r,i)=><div key={i} className="row" style={{justifyContent:"space-between",padding:"3px 0"}}><span className="mono">{Object.entries(r.params).map(([k,v])=>`${k}=${v}`).join(", ")}</span><span className="note">CV {r.meanScore.toFixed(3)}</span><button className="btn" style={{padding:"2px 8px"}} onClick={()=>setParams(p=>({...p,...r.params}))}>Apply</button></div>)}
                    </div>})()}
                  </div>
                )}
              </div>
          ...
          <div className="teach-note" style={{ marginTop: 14 }}>
            <span className="ic">🎓</span>
            <span>
              <b>How tuning works:</b> the selected search method tries different hyperparameter combinations and scores each one with cross-validation.
              <br />
              <span className="note">Grid Search checks defined combinations systematically. Random Search samples combinations. Bayesian Optimization uses earlier scores to focus on promising regions. The <b>best CV score</b> is the strongest validation result found, not a guarantee of real-world performance.</span>
            </span>
          </div>
          <div className="row" style={{ justifyContent: "flex-end", marginTop: 8 }}>
            <LearnButton topic="hyperparameters" compact />
          </div>
          <ReportCard title="Model Comparison Report" icon="🤖" exportText={modelCompareResults.length ? `# Model Comparison Report\n\nTask: ${task}\nRecommended model: ${modelCompareResults[0].algo}\nBest CV score: ${modelCompareResults[0].score.toFixed(4)} (± ${modelCompareResults[0].std.toFixed(4)})\nModels tested: ${modelCompareResults.length}\n\n${modelCompareResults.map((r, i) => `${i + 1}. ${MODEL_INFO[r.algo]?.label ?? r.algo} — CV ${Number.isFinite(r.score) ? r.score.toFixed(4) : "—"} (±${r.std.toFixed(4)}), ${r.ms.toFixed(0)}ms${r.error ? " — ERROR" : ""}`).join("\n")}\n` : undefined}>
            <div className="row" style={{justifyContent:"space-between",alignItems:"center",marginBottom:10}}><div className="note">Benchmarks every available {task} model using the current preprocessed matrix and {cvFolds}-fold cross-validation.</div><button className="btn" onClick={runModelComparison} disabled={modelCompareRunning || !built}>{modelCompareRunning ? "Comparing…" : "▶ Run model comparison"}</button></div>
            {modelCompareResults.length ? <><div className="metric-grid" style={{marginBottom:10}}><div className="metric"><span className="v">{modelCompareResults[0].algo}</span><span className="k">Recommended model</span></div><div className="metric"><span className="v">{modelCompareResults[0].score.toFixed(4)}</span><span className="k">Best CV score</span></div><div className="metric"><span className="v">{modelCompareResults.length}</span><span className="k">Models tested</span></div><div className="metric"><span className="v">{modelCompareResults[0].std.toFixed(4)}</span><span className="k">Best CV std</span></div></div><div style={{overflowX:"auto"}}><table className="dtable"><tbody><tr><th>Rank</th><th>Model</th><th>CV score</th><th>Std</th><th>Time</th><th>Status</th></tr>{modelCompareResults.map((r,i)=><tr key={r.algo}><td>{i+1}</td><td><b>{MODEL_INFO[r.algo]?.label ?? r.algo}</b></td><td>{Number.isFinite(r.score) ? r.score.toFixed(4) : "—"}</td><td>{r.std.toFixed(4)}</td><td>{r.ms.toFixed(0)} ms</td><td>{r.error ? "⚠ Error" : "✓ OK"}</td></tr>)}</tbody></table></div><div className="teach-note" style={{marginTop:10}}><span className="ic">💡</span><span><b>Recommendation:</b> {MODEL_INFO[modelCompareResults[0].algo]?.label ?? modelCompareResults[0].algo} has the strongest mean CV score. You can select it above and then tune its hyperparameters.</span></div></> : <div className="note">Run the comparison to generate a dataset-specific model benchmark and recommendation.</div>}
          </ReportCard>
          <div className="card" style={{ marginTop: 16 }}>
      ...
      {step === "validation" && ds && (
        <>
          <div className="card">
            <div className="card-h"><span className="t">How will you validate the model?</span><LearnButton topic="validation" compact /><span className="mono r">{MODEL_INFO[algo]?.label ?? algo}</span></div>
            <div className="card-b">
              <div className="row" style={{ gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
                <div className="rangebtns"><button className={valMethod === "kfold" ? "on" : ""} onClick={() => setValMethod("kfold")}>K-Fold</button><button className={valMethod === "stratified" ? "on" : ""} onClick={() => setValMethod("stratified")}>Stratified K-Fold</button><button className={valMethod === "repeated" ? "on" : ""} onClick={() => setValMethod("repeated")}>Repeated K-Fold</button><button className={valMethod === "timeseries" ? "on" : ""} onClick={() => setValMethod("timeseries")}>Time Series</button><button className={valMethod === "group" ? "on" : ""} onClick={() => setValMethod("group")}>Group K-Fold</button><button className={valMethod === "holdout" ? "on" : ""} onClick={() => setValMethod("holdout")}>Hold-out</button></div>
              </div>
              {/* split diagram */}
              {(() => { const n = built ? built.X.length : ds.nrows; const sc = valMethod === "holdout" ? splitCounts(n, valSize, testSize) : { train: n - Math.round(n * testSize), val: 0, test: Math.round(n * testSize) }; const pct = (x: number) => `${(x / n) * 100}%`; return (
                <>
                  ...
                  <div className="row" style={{ gap: 18, flexWrap: "wrap", marginTop: 12 }}>
                    ...
                    {valMethod !== "holdout" && <div className="knob" style={{ margin: 0, minWidth: 190 }}><div className="kr"><span>CV folds (k)</span><b>{cvFolds}</b></div><input type="range" min={2} max={10} value={cvFolds} onChange={(e) => setCvFolds(+e.target.value)} /></div>}
                    {valMethod === "group" && <div className="ec"><label className="fld">Group column</label><select value={cvGroupCol} onChange={e=>setCvGroupCol(e.target.value)}><option value="">Select group</option>{ds.columns.filter(c=>c.name!==target).map(c=><option key={c.name}>{c.name}</option>)}</select></div>}
                    {valMethod === "repeated" && <div className="knob" style={{margin:0,minWidth:190}}><div className="kr"><span>Repeats</span><b>{cvRepeats}</b></div><input type="range" min={2} max={5} value={cvRepeats} onChange={e=>setCvRepeats(+e.target.value)}/></div>}
                  </div>
                  <div className="note" style={{ marginTop: 10 }}>{valMethod === "kfold" ? `K-fold: ${cvFolds} folds.` : valMethod === "stratified" ? `Stratified K-fold preserves class proportions across ${cvFolds} folds.` : valMethod === "repeated" ? `Repeated K-fold runs ${cvRepeats} rounds × ${cvFolds} folds for a more stable estimate.` : valMethod === "timeseries" ? `Time-series validation keeps chronological order and never trains on future rows.` : valMethod === "group" ? `Group K-fold keeps rows from the same group together in one fold.` : "Hold-out keeps a validation slice aside and a separate test slice untouched until the end."}</div>
                </>
              ); })()}
            </div>
          </div>
          ...
          {valMethod !== "holdout" && (
            <div className="card" style={{ marginTop: 16 }}>
              ...
              <div className="card-b">
                <div className="row" style={{ marginBottom: 14 }}><button className="btn" onClick={runCrossVal} disabled={cvRunning || !built}>▶ Run {valMethod === "repeated" ? `${cvRepeats}×${cvFolds}` : cvFolds}-fold validation</button><span className="note">trains a fresh model on each fold — {task === "classification" ? "accuracy" : "R²"} per fold</span></div>
        ...
        <>
          ...
          {result && (<>
            ...
            <div className="split col-2e">
              ...
              <div className="card"><div className="card-h"><span className="t">Feature importance &amp; explainability</span></div>
                <div className="card-b">
                  {result.importance && <label className="fld">Model coefficients / built-in importance</label>}
                  ...
                  {!result.importance && <div className="note">This model exposes no per-feature coefficients (e.g. KNN / Naive Bayes) — see permutation importance below instead.</div>}
                  <label className="fld" style={{ marginTop: result.importance ? 14 : 0 }}>Permutation importance (model-agnostic XAI)</label>
                  <div className="note" style={{ marginBottom: 8 }}>How much held-out {task === "classification" ? "accuracy" : "R²"} drops when a feature&apos;s values are shuffled — works for every model here, the same idea SHAP/LIME are built on.</div>
                  {result.permImp.map((f) => <div key={f.name} style={{ marginBottom: 8, fontSize: 12 }}>{f.name} <span className="mono r">{f.importance.toFixed(3)}</span><div className="impbar"><i style={{ width: `${Math.max(0, Math.round(f.importance * 100))}%` }} /></div></div>)}
                </div>
              </div>
            </div>
      ...
      {step === "deploy" && ds && result && (
        <>
          ...
          <div className="card" style={{ marginTop: 16 }}>
            <div className="card-h"><span className="t">🧠 Explainability &amp; ML lifecycle</span><span className="mono r">real model outputs</span></div>
            <div className="card-b">
              <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
                <button className="btn sm" onClick={runExplainability}>Run SHAP + LIME</button>
                <select value={driftMethod} onChange={e => setDriftMethod(e.target.value as typeof driftMethod)}><option>PSI</option><option>KS</option><option>JS</option><option>Wasserstein</option></select>
                <button className="btn ghost sm" onClick={runDriftCheck}>Check feature drift</button>
                <button className="btn ghost sm" onClick={saveLifecycleVersion}>Save model version</button>
                <button className="btn ghost sm" onClick={compareLatestVersions}>Champion vs challenger</button>
              </div>
              <div className="note" style={{ marginTop: 8 }}>SHAP uses a Monte-Carlo Shapley approximation against the training background; LIME fits a weighted local surrogate. Drift compares the earlier 70% of the loaded matrix with the latest 30%.</div>
              {lifecycleMsg && <div className="teach-note" style={{ marginTop: 10 }}><span className="ic">ℹ️</span><span>{lifecycleMsg}</span></div>}
              {shapOut && <div style={{ marginTop: 14 }}><label className="fld">SHAP — global/local contribution view · base {shapOut.baseValue.toFixed(4)} → prediction {shapOut.prediction.toFixed(4)}</label><div className="checklist">{shapOut.rows.slice(0, 10).map(r => <span className="chip" key={r.feature}>{r.feature}: <b>{r.contribution >= 0 ? "+" : ""}{r.contribution.toFixed(4)}</b></span>)}</div><div className="note">Positive values push the selected prediction upward; negative values push it downward. {shapOut.samples} random feature orderings were evaluated.</div></div>}
              {limeOut && <div style={{ marginTop: 14 }}><label className="fld">LIME — local surrogate · R² {limeOut.r2.toFixed(3)} · {limeOut.samples} perturbations</label><div className="checklist">{limeOut.rows.slice(0, 10).map(r => <span className="chip" key={r.feature}>{r.feature}: <b>{r.contribution >= 0 ? "+" : ""}{r.contribution.toFixed(4)}</b></span>)}</div><div className="note">These are local surrogate contributions, not model coefficients.</div></div>}
              {driftOut.length > 0 && <div style={{ marginTop: 14 }}><label className="fld">Feature drift — {driftMethod}</label><div className="checklist">{driftOut.slice(0, 10).map(r => <span className={`chip ${r.drifted ? "on" : ""}`} key={r.feature}>{r.feature}: {r.score.toFixed(4)} {r.drifted ? "⚠ drift" : "✓ stable"}</span>)}</div></div>}
              {versions.length > 0 && <div style={{ marginTop: 14 }}><label className="fld">Local model registry</label><div className="note">{versions.map(v => `${v.version} · ${v.algorithm} · ${v.status} · ${new Date(v.createdAt).toLocaleString()}`).join("\n")}</div></div>}
            </div>
          </div>
      ...
      <div className={`modal-wrap ${learnTopic ? "show" : ""}`} onClick={(e) => { if (e.target === e.currentTarget) setLearnTopic(null); }}>
        {learnTopic && (
          <div className="modal" role="dialog" aria-modal="true" aria-labelledby="ml-learn-title">
            <div className="mh">
              <b id="ml-learn-title">📚 Learn: {LEARN_TOPICS[learnTopic].title}</b>
              <button className="x" onClick={() => setLearnTopic(null)}>×</button>
            </div>
            <div className="mb">
              <div className="teach-note"><span className="ic">💡</span><span><b>What is it?</b> {LEARN_TOPICS[learnTopic].what}</span></div>
              <div className="teach-note" style={{ marginTop: 10 }}><span className="ic">🎯</span><span><b>Why do we use it?</b> {LEARN_TOPICS[learnTopic].why}</span></div>
              <div className="teach-note" style={{ marginTop: 10 }}><span className="ic">🔬</span><span><b>How does it work?</b> {LEARN_TOPICS[learnTopic].how}</span></div>
              <div className="note" style={{ marginTop: 12, lineHeight: 1.6 }}><b>Key takeaway:</b> {LEARN_TOPICS[learnTopic].takeaway}</div>
            </div>
          </div>
        )}
      </div>
